...
export async function verifySession(sessionToken, env) {
  if (!sessionToken || (!env.DB && !env.img_url)) return false;

  // 1. 优先从 D1 验证
  if (env.DB) {
    try {
      const row = await env.DB.prepare('SELECT user, expires_at FROM sessions WHERE token = ?').bind(sessionToken).first();
      if (row) {
      ...
      console.warn('D1 verifySession error, trying KV fallback:', e.message);
    }
  }
  ...
  if (env.img_url) {
    try {
      ...
      console.error('KV session verify error:', e);
      return false;
    }
  }
...
export async function createSession(user, env) {
  ...
  const sessionData = { user, createdAt, expiresAt };

  if (env.DB) {
    try {
      await env.DB.prepare(
        'INSERT INTO sessions (token, user, created_at, expires_at) VALUES (?, ?, ?, ?)'
      ).bind(token, user, createdAt, expiresAt).run();
    } catch (e) {
      console.warn('D1 createSession error:', e.message);
    }
  }
  ...
  if (env.img_url) {
    try {
      ...
      console.warn('KV session write warning:', kvErr.message);
    }
  }
...
export async function deleteSession(sessionToken, env) {
  ...
  if (env.DB) {
    try {
      ...
      console.warn('D1 deleteSession error:', e.message);
    }
  }
  if (env.img_url) {
    await env.img_url.delete(`session:${sessionToken}`).catch(() => {});
  }
}
